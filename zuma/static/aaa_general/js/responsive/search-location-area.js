...
$(document).ready(function() {	
	...
	if($("#cityarea" ).attr("type") != 'text') {		
		...
		$('#cityarea').blur(function(){
			setTimeout(function(){
		   		 if($('.drop-down-select').length){
			    	$('.drop-down-select').hide();
			    }else {
			    	$('#listOfAreas').hide();
			    }
			}, 500);
		...
		var dropDownWrapSelectFirstText = $('.drop-down-wrap-select .drop-down').text();
		$('#citylist').change(function(){
			if($('.drop-down-select').length){
				$('.drop-down-wrap-select .drop-down').text(dropDownWrapSelectFirstText);
				var cityID = $('#citylist').val(); 
				if (cityID != null && cityID != '' && cityID != undefined) {
					getAreaByCity(cityID, function(){
						$('#listOfAreas').show().html("");
		...
		$("#listOfAreas").delegate(".city-area-list > li", "click" , function() {  
		    ...
		    if($('.drop-down-select').length){
		    	$('.drop-down-wrap-select >  .drop-down').text($(this).find('a').html());
		    	$('.drop-down-select').hide();
		    }else {
		    	$('#listOfAreas').hide();
		    }
		});	
		$('.drop-down-wrap-select > .drop-down').click(function(){
			$(".drop-down-select").fadeToggle(100);
		})
